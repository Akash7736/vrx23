...
class PDController(Node):
    ...
    def __init__(self):
        ...
        self.subscriptiongoal = self.create_subscription(PoseStamped,'/vrx/stationkeeping/goal',self.goal_callback,10)
...
# yawmsg = Float64(self.yaw)
        ...
        self.eyaw = self.yawg - self.yaw 
        self.update(self.ex,self.ey,self.eyaw,data.angular_velocity.z)

    def goal_callback(self,data:PoseStamped):
        self.xg = data.pose.position.x 
        self.yg = data.pose.position.y
        quat = [data.pose.orientation.x,data.pose.orientation.y,data.pose.orientation.z,data.pose.orientation.w]
        self.yawg = euler_from_quaternion(quat)[2]

    # def goal_callback(self,data:PoseStamped):  # if goal has lat and lon
    #     self.xg,self.yg = self.gps2enu(origin[0],origin[1],data.pose.position.x,data.pose.position.y) 
    #     quat = [data.pose.orientation.x,data.pose.orientation.y,data.pose.orientation.z,data.pose.orientation.w]
    #     self.yawg = euler_from_quaternion(quat)[2]
    #     print(f"xg={self.xg}  yg={self.yg}")
