...
class VoxelProcess(Node):

    def __init__(self):
        super().__init__('VoxelProcess')
    ...
    #   '/wamv/wamv/lidar_wamv_link').get_parameter_value().string_value
        ...
        self.meshpublisher = self.create_publisher(PointCloud2, '/mesh_points', 10)
    ...
    def voxel_callback(self, msg:PointCloud2):
        ...
        for point in point_cloud2.read_points(msg, skip_nans=True):
            ...
            if point_wrt_world[2]  > self.heightthresh and point_wrt_world[2] < 1.5 and pt2[0]<20.0 and pt2[1]<20.0:
                ...
                gridset.add((round(point_wrt_world[0],1),round(point_wrt_world[1],1),0))

        # x_coords, y_coords = zip(*gridset)
                
        pointsincam = np.array(pointsincam)

        pt_arr = np.array(list(gridset))
        ...
        self.meshpublisher.publish(pc_msg)
        print(pt_arr.shape)
        print("Mesh Points Published ...........")
