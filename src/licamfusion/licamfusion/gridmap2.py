...
class Imidata:
    def __init__(self,timestamp,grid,action,latitude=None,longitude = None, yaw = None, x = None, y = None):
...
class FrameListener(Node):
    ...
    def __init__(self):
        ...
        self.voxelsub = message_filters.Subscriber(self,PointCloud2,'/myvoxel')
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer([self.voxelsub, self.act_sub,self.gps_sub,self.imu_sub], 10,0.1)
    ...
    def callback(self,voxelmsg, act,gps,imu:Imu):
        ...
        self.roll = int(self.roll_rad *(180/np.pi))
        self.pitch = int(self.pitch_rad *(180/np.pi))
        self.yaw = int(self.yaw_rad *(180/np.pi))
        ...
        if self.recflag==True:
            if count_files(f'{self.main_directory}/pickle_files')<600:
                ctime = self.currtime
                # time.sleep(3)
                ...
                subfolders = ['pickle_files','gridmaps']
